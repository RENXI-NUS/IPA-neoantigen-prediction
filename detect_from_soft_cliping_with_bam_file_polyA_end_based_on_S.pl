...
open(my $IN, "-|", "samtools view -F 4 -f 0x2 $sam_file")or die $!;
...
if($field1[0] eq $field2[0]){
		...
		my $seq = substr($field2[9],0,length($field2[9]) - $index);
		print OUT $field2[2];
	        print OUT "\t";
        	print OUT ($field2[3]+(length($seq))-1);
	        print OUT "\t";
		print OUT "+";
        	print OUT "\t";
		print OUT $field2[5]."\t";
